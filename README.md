...
- **`ModuleNotFoundError: No module named 'duckdb'` al pasar `verificacion_b2.sh`** → el
  ...
  se entera. Lo arregla `bash aula/preparar_bloque2.sh`, o a mano
...
- **Después de un `docker compose down`, el aula vuelve a no encontrar duckdb** → es correcto: lo
  que se instala en un contenedor muere con él. Vuelve a pasar `aula/preparar_bloque2.sh` antes de
...
└── aula/                      ← provisión de puestos, publicación e imágenes por USB
    ...
    ├── preparar_bloque2.sh    ← ritual de apertura del Bloque 2: sincroniza, repara y verifica
...
Apertura de bloque (alumnado y docente): `bash aula/preparar_bloque2.sh` · detalle de la
sincronización: `aula/ACTUALIZAR.md` · publicación del docente: `aula/GITHUB.md`
